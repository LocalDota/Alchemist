...
--Полное комбо
	...
	if Menu.IsKeyDown(Elder.optionFullCombo) and enemy then
...
--Второе комбо
	if Menu.IsKeyDown(Elder.optionCombo2) and enemy then
...
function Elder.Blink(ability, origin, distance, mana)
	if Menu.IsEnabled(Elder.optionBlink) and Ability.IsReady(ability) and Ability.IsCastable(ability, mana) and distance > 700 then
    	Ability.CastPosition(ability, origin)
    	return true
    end	
end
