...
--Авто-Roar
	...
	if Menu.IsEnabled(Druid.optionAutoRoarEnable) then
		...
		if Menu.IsEnabled(Druid.optionAutoRoarDruidChan) and  ((Entity.GetOrigin(nearestHero) - Entity.GetOrigin(myHero)):Length2D()) < 375 and  NPC.IsChannellingAbility(nearestHero) and nearestHero then
			Druid.AutoRoar(roar, mana)
		end	
		--Если возле мишки кастуется заклинание
		if Menu.IsEnabled(Druid.optionAutoRoarBearChan) and bear and  ((Entity.GetOrigin(nearestHero) - Entity.GetOrigin(bear)):Length2D()) < 375 and  NPC.IsChannellingAbility(nearestHero) and nearestHero then
			Druid.AutoRoar(roarB, manaB)
		end		
	end	
...
--Преследовать курсор
	if Menu.IsEnabled(Druid.optionCursor) then
		if Menu.IsKeyDown(Druid.optionFullCombo) and (not enemy) and bear then
			NPC.MoveTo(myHero, Input.GetWorldCursorPos())
			NPC.MoveTo(bear, Input.GetWorldCursorPos())		
		end	

		if Menu.IsKeyDown(Druid.optionComboB) and (not enemy) and bear then
			NPC.MoveTo(bear, Input.GetWorldCursorPos())	
		end
	end		
end	
