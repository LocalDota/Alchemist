...
--Комбо
    if Menu.IsKeyDown(Sven.optionFullCombo) and enemy and (not NPC.IsChannellingAbility(myHero)) and (not NPC.GetModifier(enemy, "modifier_black_king_bar_immune")) and (not NPC.GetModifier(enemy, "modifier_life_stealer_rage")) and (not NPC.GetModifier(enemy, "modifier_juggernaut_blade_fury")) and not NPC.GetModifier(enemy, "modifier_item_lotus_orb_active") and not NPC.GetModifier(enemy, "modifier_antimage_counterspell")  then
...
--Без ульты
    if Menu.IsKeyDown(Sven.optionCombo) and enemy and (not NPC.IsChannellingAbility(myHero)) and (not NPC.GetModifier(enemy, "modifier_black_king_bar_immune")) and (not NPC.GetModifier(enemy, "modifier_life_stealer_rage")) and (not NPC.GetModifier(enemy, "modifier_juggernaut_blade_fury")) and not NPC.GetModifier(enemy, "modifier_item_lotus_orb_active") and not NPC.GetModifier(enemy, "modifier_antimage_counterspell")  then
        local distance = ((Entity.GetOrigin(myHero) - (Entity.GetOrigin(enemy))):Length2D())
        local origin = Entity.GetOrigin(enemy)
        local value = Menu.GetValue(Sven.optionBlinkSlider)
...
function Sven.Storm(ability, mana, enemy)
    if Menu.IsEnabled(Sven.optionStorm) and Ability.IsReady(ability) and Ability.IsCastable(ability, mana) and not Ability.IsInAbilityPhase(ability) then
...
function Sven.Warcry(ability, mana)
    if Menu.IsEnabled(Sven.optionWarcry) and Ability.IsReady(ability) and Ability.IsCastable(ability, mana) and not Ability.IsInAbilityPhase(ability)  then
...
function Sven.God(ability, mana)
    if Menu.IsEnabled(Sven.optionGod) and Ability.IsReady(ability) and Ability.IsCastable(ability, mana) and not Ability.IsInAbilityPhase(ability)  then
        Ability.CastNoTarget(ability, mana)
        return true
    end 
end
