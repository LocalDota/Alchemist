...
--Полное комбо
	...
	if Menu.IsKeyDown(Druid.optionFullCombo) and enemy and (not NPC.IsChannellingAbility(myHero)) and (not NPC.IsChannellingAbility(bear)) then
		...
		if Menu.IsEnabled(Druid.optionBloodthornB) and Druid.ItemTarget(bloodthornB, enemy, manaB, bear) == true then
			return
		end
		...
		if Menu.IsEnabled(Druid.optionBloodthorn) and Druid.ItemTarget(bloodthorn, enemy, mana, myHero) == true then
			return
		end
