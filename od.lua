...
local lastMoveOrder8 = 0
local orderDelay8 = 0.1 

local lastMoveOrder9 = 0
local orderDelay9 = 0.1

local lastMoveOrder10 = 0
local orderDelay10 = 1

local lastMoveOrder11 = 0
local orderDelay11 = 1  

local lastMoveOrder12 = 0
local orderDelay12 = 1

local lastMoveOrder13 = 0
local orderDelay13 = 3 

local lastMoveOrder14 = 0
local orderDelay14 = 0.1

local lastMoveOrder15 = 0
local orderDelay15 = 3
...
OD.optionMeteorCombo = Menu.AddKeyOption(settingsPath, "Сombo astral + meteor", Enum.ButtonCode.KEY_NONE)
Menu.AddOptionIcon(OD.optionMeteorCombo, "~/MenuIcons/enemy_evil.png")
...
{"meteor", "panorama/images/items/meteor_hammer_png.vtex_c", true},
}, false)
...
--Предметы
    ...
    local meteor = OD.CheckItem("item_meteor_hammer")

    local blinkL = OD.CheckItem("item_shift_blink")

	local blinkS = OD.CheckItem("item_overwhelming_blink")

	local blinkI = OD.CheckItem("item_arcane_blink")
...
--Полное комбо
    ...
    if Menu.IsKeyDown(OD.optionFullCombo) and enemy and (not NPC.IsChannellingAbility(myHero)) and (not NPC.GetModifier(enemy, "modifier_black_king_bar_immune")) and (not NPC.GetModifier(enemy, "modifier_life_stealer_rage")) and (not NPC.GetModifier(enemy, "modifier_juggernaut_blade_fury")) and not NPC.GetModifier(enemy, "modifier_item_lotus_orb_active") and not NPC.GetModifier(enemy, "modifier_antimage_counterspell")  then
        ...
        local modifierAstral = NPC.GetModifier(enemy, 'modifier_obsidian_destroyer_astral_imprisonment_prison')	
        if modifierAstral then
        	local timer = Modifier.GetDieTime(modifierAstral) - GameRules.GetGameTime()
        	if timer < (2.9 - NetChannel.GetAvgLatency(Enum.Flow.FLOW_OUTGOING)) then
        		if (Menu.IsSelected(OD.itemsSelection, "meteor")) and OD.ItemOrigin(meteor, origin, mana) == true then
        			return
        		end
        	end
        	return 
        ...
        if meteor then
        	if (not Ability.IsChannelling(meteor)) then
        		...
        		if OD.Blink(blinkI, origin, distance, value) == true then
        		    return
        		end

        		if OD.Blink(blinkS, origin, distance, value) == true then
        		    return
        		end

        		if OD.Blink(blinkL, origin, distance, value) == true then
        		    return
        		end
        	...
        	if NPC.IsLinkensProtected(enemy) then
        	    ...
        	    for i = 1, #TableLinken do
        	        local item = items[TableLinken[i]]
        	        if (Menu.IsSelected(OD.linkenSelection, TableLinken[i])) then
        	        	if OD.SleepReady(orderDelay14, lastMoveOrder14) then
        	            	if OD.Breaker(item, enemy, mana) == true then
        	            	    return
        	            	end 
        	            	lastMoveOrder14 = os.clock()
        	            end	    
        	        end
        	    end  
        	end
        	...
        	if scepter or NPC.GetModifier(myHero, "modifier_item_ultimate_scepter_consumed") then
        	    if Menu.IsEnabled(OD.optionCurrent) then
        	        ...
        	        if OD.SleepReady(orderDelay13, lastMoveOrder13) then
        	            if Menu.IsEnabled(OD.optionAstral) and Menu.GetValue(OD.optionTurn) == 0 and (not NPC.GetModifier(enemy, "modifier_bloodthorn_debuff")) and (not NPC.GetModifier(enemy, "modifier_orchid_malevolence_debuff")) and not NPC.GetModifier(enemy, "modifier_sheepstick_debuff") and not NPC.GetModifier(enemy, "modifier_rod_of_atos_debuff") and not NPC.IsStunned(enemy) and OD.Astral(astral, mana, enemy) == true then
        	                return
        	            end
        	            lastMoveOrder13 = os.clock()
        	        end        
        	    end
        	end
        	...
        	if Menu.IsEnabled(OD.optionAstralMove) and NPC.GetModifier(enemy, "modifier_obsidian_destroyer_astral_imprisonment_prison") then
        	    if OD.SleepReady(orderDelay11, lastMoveOrder11) then
        	        NPC.MoveTo(myHero, origin)
        	        lastMoveOrder11 = os.clock()
        	    end    
        	end 
        	...
        	if NPC.GetModifier(myHero, "modifier_item_hurricane_pike_range") then
        	    if OD.SleepReady(orderDelay12, lastMoveOrder12) then
        	        if not Ability.GetAutoCastState(orb) then
        	            Ability.ToggleMod(orb)
        	            return
        	        end
        	        lastMoveOrder12 = os.clock()
        	    end    
        	    if OD.SleepReady(orderDelay9, lastMoveOrder9) then
        	        if Ability.GetAutoCastState(orb) then
        	            Player.AttackTarget(myPlayer, myHero, enemy)
        	        end    
        	        lastMoveOrder9 = os.clock()
        	    end 
        	    return   
        	end 
        	...
        	if Menu.IsEnabled(OD.optionAstralMove) and NPC.GetModifier(enemy, "modifier_obsidian_destroyer_astral_imprisonment_prison") then
        	    if OD.SleepReady(orderDelay10, lastMoveOrder10) then
        	        NPC.MoveTo(myHero, origin)
        	        lastMoveOrder10 = os.clock()
        	    end    
        	end    
        	...
        	if OD.SleepReady(orderDelay8, lastMoveOrder8) then
        	    Player.AttackTarget(myPlayer, myHero, enemy)
        	    lastMoveOrder8 = os.clock()
        	end
        end			 
    end

    if Menu.IsKeyDown(OD.optionMeteorCombo) and enemy and (not NPC.IsChannellingAbility(myHero)) and (not NPC.GetModifier(enemy, "modifier_black_king_bar_immune")) and (not NPC.GetModifier(enemy, "modifier_life_stealer_rage")) and (not NPC.GetModifier(enemy, "modifier_juggernaut_blade_fury"))  then
		if meteor then
			if (not Ability.IsChannelling(meteor)) then

				distance = ((Entity.GetOrigin(myHero) - (Entity.GetOrigin(enemy))):Length2D())
				origin = Entity.GetOrigin(enemy)
				value = Menu.GetValue(OD.optionBlinkSlider)
				if OD.Blink(blink, origin, distance, value) == true then
				    return
				end
				...
				if scepter or NPC.GetModifier(myHero, "modifier_item_ultimate_scepter_consumed") then
				    if Menu.IsEnabled(OD.optionCurrent) then
				        if Ability.GetCurrentCharges(astral) > 1 then
				            if Menu.IsEnabled(OD.optionAstral) and (not NPC.GetModifier(enemy, "modifier_bloodthorn_debuff")) and (not NPC.GetModifier(enemy, "modifier_orchid_malevolence_debuff")) and not NPC.GetModifier(enemy, "modifier_sheepstick_debuff") and not NPC.GetModifier(enemy, "modifier_rod_of_atos_debuff") and not NPC.IsStunned(enemy) and OD.Astral(astral, mana, enemy) == true then
				                return
				            end
				        end
				    else
				        if OD.SleepReady(orderDelay15, lastMoveOrder15) then
				            if Menu.IsEnabled(OD.optionAstral)  and (not NPC.GetModifier(enemy, "modifier_bloodthorn_debuff")) and (not NPC.GetModifier(enemy, "modifier_orchid_malevolence_debuff")) and not NPC.GetModifier(enemy, "modifier_sheepstick_debuff") and not NPC.GetModifier(enemy, "modifier_rod_of_atos_debuff") and not NPC.IsStunned(enemy) and OD.Astral(astral, mana, enemy) == true then
				                return
				            end
				            lastMoveOrder15 = os.clock()
				        end        
				    end
				end
...
--Преследовать курсор
    ...
    if Menu.IsEnabled(OD.optionCursor) then
        if Menu.IsKeyDown(OD.optionMeteorCombo) then
            if (not enemy) and (not NPC.IsChannellingAbility(myHero)) then
                NPC.MoveTo(myHero, Input.GetWorldCursorPos())
            end     
        end 
