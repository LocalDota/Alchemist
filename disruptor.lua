...
{"gungir", "panorama/images/items/gungir_png.vtex_c", true},
...
Menu.AddMenuIcon(originPath, "~/MenuIcons/globe_world.png")

local optionWhoDraw = Menu.AddOptionCombo(originPath, "What enemies to draw?", { "Search all", "Only target"}, 1)
Menu.AddOptionIcon(optionWhoDraw, "~/MenuIcons/ellipsis.png")

local optionRangeDraw = Menu.AddOptionSlider(originPath, "In what range to search(function 'search all')", 1, 10000, 2000)
Menu.AddOptionIcon(optionRangeDraw, "~/MenuIcons/edit.png")
...
function Disruptor.OnUpdate()
	...
	local gungir = Disruptor.CheckItem("item_gungir")
    ...
    if enemy and not NPC.IsPositionInRange(enemy, Input.GetWorldCursorPos(), Menu.GetValue(optionRangeToTarget), 0) then
        enemy = nil
        if Menu.GetValue(optionWhoDraw) == 1 then
...
--Thunder
	...
	if Menu.IsKeyDown(optionFullCombo) and enemy and (not NPC.IsChannellingAbility(myHero)) and (not NPC.GetModifier(enemy, "modifier_black_king_bar_immune")) then
        ...
        if glimpsed[1] then
        	if (GameRules.GetGameTime() - glimpsed[2]) > 2 then
        ...
        if glimpsed[1] then
        	if (GameRules.GetGameTime() - glimpsed[2]) > 2 then
        		if (Menu.IsSelected(Disruptor.itemsSelection, "gungir")) and Disruptor.ItemTarget(gungir, enemy, mana) == true then
            		return
            	end
            end
        else
        ...
        local position =  Disruptor.GetPredictedPosition(enemy, 0.6)
        if (Menu.IsSelected(Disruptor.skillsSelection, "kinetic")) and position and Ability.SecondsSinceLastUse(glimpse) > 1 and Disruptor.Kinetic(kinetic, position, mana) == true then
        	return
        end	


        if (Menu.IsSelected(Disruptor.skillsSelection, "storm")) and Ability.SecondsSinceLastUse(glimpse) > 1 and Disruptor.Storm(storm, position, mana) == true then
        	return
        end		
...
function Disruptor.OnMenuOptionChange(option, oldValue, newValue)
    if option == slider then
        ...
        if glimpseParticle2 then
            Particle.Destroy(glimpseParticle2)
            glimpseParticle2 = nil
            ...
            glimpseParticle2 = Particle.Create("particles\\ui_mouseactions\\drag_selected_ring.vpcf")
        	Particle.SetControlPoint(glimpseParticle2, 0, glimpseOrigin2)
        	Particle.SetControlPoint(glimpseParticle2, 1, Vector(tempColor.r, tempColor.g, tempColor.b))
            Particle.SetControlPoint(glimpseParticle2, 2, Vector(Menu.GetValue(slider)*1.0923, 255, 255))
            Particle.SetControlPoint(glimpseParticle2, 3, Vector(255 - tempColor.a, 0, 0))
        end
        if glimpseParticle3 then
            Particle.Destroy(glimpseParticle3)
            glimpseParticle3 = nil
            ...
            glimpseParticle3 = Particle.Create("particles\\ui_mouseactions\\drag_selected_ring.vpcf")
        	Particle.SetControlPoint(glimpseParticle3, 0, glimpseOrigin3)
        	Particle.SetControlPoint(glimpseParticle3, 1, Vector(tempColor.r, tempColor.g, tempColor.b))
            Particle.SetControlPoint(glimpseParticle3, 2, Vector(Menu.GetValue(slider)*1.0923, 255, 255))
            Particle.SetControlPoint(glimpseParticle3, 3, Vector(255 - tempColor.a, 0, 0))
        end
        if glimpseParticle4 then
            Particle.Destroy(glimpseParticle4)
            glimpseParticle4 = nil
            ...
            glimpseParticle4 = Particle.Create("particles\\ui_mouseactions\\drag_selected_ring.vpcf")
        	Particle.SetControlPoint(glimpseParticle4, 0, glimpseOrigin4)
        	Particle.SetControlPoint(glimpseParticle4, 1, Vector(tempColor.r, tempColor.g, tempColor.b))
            Particle.SetControlPoint(glimpseParticle4, 2, Vector(Menu.GetValue(slider)*1.0923, 255, 255))
            Particle.SetControlPoint(glimpseParticle4, 3, Vector(255 - tempColor.a, 0, 0))
        end
        if glimpseParticle5 then
            Particle.Destroy(glimpseParticle5)
            glimpseParticle5 = nil
            ...
            glimpseParticle5 = Particle.Create("particles\\ui_mouseactions\\drag_selected_ring.vpcf")
        	Particle.SetControlPoint(glimpseParticle5, 0, glimpseOrigin5)
        	Particle.SetControlPoint(glimpseParticle5, 1, Vector(tempColor.r, tempColor.g, tempColor.b))
            Particle.SetControlPoint(glimpseParticle5, 2, Vector(Menu.GetValue(slider)*1.0923, 255, 255))
            Particle.SetControlPoint(glimpseParticle5, 3, Vector(255 - tempColor.a, 0, 0))
        end
    end
end        
...
function Disruptor.OnDraw()
	...
	if Menu.IsEnabled(optionGlimpseEnable) and glimpse and Ability.GetLevel(glimpse) >= 1 then
		...
		if Menu.GetValue(optionWhoDraw) == 0 then
			for i = 1, Heroes.Count() do
			    local enemyAll = Heroes.Get(i)
			    do
			    	if Entity.IsDormant(enemyAll) or not NPC.IsEntityInRange(myHero, enemyAll, Menu.GetValue(optionRangeDraw)) or  not Entity.IsHero(enemyAll) or not Entity.IsAlive(enemyAll) or Entity.IsSameTeam(myHero, enemyAll) or NPC.IsIllusion(enemyAll) then
			        ...
			        if enemyAll and Entity.IsHero(enemyAll) and NPC.IsEntityInRange(myHero, enemyAll, Menu.GetValue(optionRangeDraw)) and Entity.IsAlive(enemyAll) and not Entity.IsSameTeam(myHero, enemyAll) and not NPC.IsIllusion(enemyAll) and not Entity.IsDormant(enemyAll) then
			        	if i == 1 or i == 6 then
			        		...
			        		if Menu.IsEnabled(optionGlimpseEnable) and glimpse and Ability.GetLevel(glimpse) >= 1 then
			        			if enemyAll then
			        				if Disruptor.SleepReady(orderDelay2, lastMoveOrder2) then
			        					...
			        					elements[40] = Entity.GetOrigin(enemyAll)
			        					lastMoveOrder2 = os.clock()
			        				end
			        			end
			        		end
							...
							if not glimpseOrigin or elements[1] ~= glimpseOrigin or not enemyAll or not Ability.IsReady(glimpse) then
								Particle.Destroy(glimpseParticle)			
								glimpseParticle = 0
								glimpseOrigin = elements[1]
								image = nil
							else
								if glimpseParticle == 0 and enemyAll then
									glimpseParticle = Particle.Create("particles\\ui_mouseactions\\drag_selected_ring.vpcf")				
								end
								if glimpseParticle ~= 0 then
									if Menu.IsEnabled(optionCircle) then
			    			        ...
			    			        if Menu.IsEnabled(optionLine) then
			    			        	x1, y1 = Renderer.WorldToScreen(glimpseOrigin)
			    			        	x2, y2 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))
			    			        	Renderer.SetDrawColor(tempColor2.r, tempColor2.g, tempColor2.b)
			    			        	Renderer.DrawLine(x1, y1, x2, y2)
			    			        end
			    			        if Menu.IsEnabled(optionIcon) then
			    			        	x1, y1 = Renderer.WorldToScreen(glimpseOrigin)
			    			        	x2, y2 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))	
			    			        	local image = Renderer.LoadImage("panorama/images/heroes/icons/"..NPC.GetUnitName(enemyAll).."_png.vtex_c")
			    			        	Renderer.SetDrawColor(255, 255, 255)
			    			        	Renderer.DrawImage(image, x1, y1)
			    			        end	
								end
						...
						elseif i == 2 or i == 7 then
							if Menu.IsEnabled(optionGlimpseEnable) and glimpse and Ability.GetLevel(glimpse) >= 1 then
			        			if enemyAll then
			        				if Disruptor.SleepReady(orderDelay4, lastMoveOrder4) then
			        					for i = 1, 39 do
			        						elements2[i] = elements2[i+1]
			        					end	
			        					elements2[40] = Entity.GetOrigin(enemyAll)
			        					lastMoveOrder4 = os.clock()
			        				end
			        			end
			        		end

							local glimpseOrigin2 = elements2[1]
							local tempColor = Menu.GetValue(color)
							local tempColor2 = Menu.GetValue(color2)
							if (tempColor.r < 50) and (tempColor.g < 50) and (tempColor.b < 50) then
							    tempColor.r = 50
							...
							if not glimpseOrigin2 or elements2[1] ~= glimpseOrigin2 or not enemyAll or not Ability.IsReady(glimpse) then
								Particle.Destroy(glimpseParticle2)			
								glimpseParticle2 = 0
								glimpseOrigin2 = elements2[1]
								image2 = nil
							else
								if glimpseParticle2 == 0 and enemyAll then
									glimpseParticle2 = Particle.Create("particles\\ui_mouseactions\\drag_selected_ring.vpcf")				
								end
								if glimpseParticle2 ~= 0 then
									if Menu.IsEnabled(optionCircle) then
										Particle.SetControlPoint(glimpseParticle2, 0, glimpseOrigin2)
			    			        	Particle.SetControlPoint(glimpseParticle2, 1, Vector(tempColor.r, tempColor.g, tempColor.b))
			    			        	Particle.SetControlPoint(glimpseParticle2, 2, Vector(Menu.GetValue(slider)*1.0923, 255, 0))
			    			        	Particle.SetControlPoint(glimpseParticle2, 3, Vector(255 - tempColor.a, 0, 0))
			    			        end
			    			        if not Ability.IsReady(glimpse) or not Menu.IsEnabled(optionCircle) then
			    			        	Particle.SetControlPoint(glimpseParticle2, 0, nil)	
			    			        end	
			    			        if Menu.IsEnabled(optionLine) then
			    			        	x12, y12 = Renderer.WorldToScreen(glimpseOrigin2)
			    			        	x22, y22 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))
			    			        	Renderer.SetDrawColor(tempColor2.r, tempColor2.g, tempColor2.b)
			    			        	Renderer.DrawLine(x12, y12, x22, y22)
			    			        end
			    			        if Menu.IsEnabled(optionIcon) then
			    			        	x12, y12 = Renderer.WorldToScreen(glimpseOrigin2)
			    			        	x22, y22 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))	
			    			        	local image2 = Renderer.LoadImage("panorama/images/heroes/icons/"..NPC.GetUnitName(enemyAll).."_png.vtex_c")
			    			        	Renderer.SetDrawColor(255, 255, 255)
			    			        	Renderer.DrawImage(image2, x12, y12)
			    			        end	
								end
							end
						elseif i == 3 or i == 8 then
							if Menu.IsEnabled(optionGlimpseEnable) and glimpse and Ability.GetLevel(glimpse) >= 1 then
			        			if enemyAll then
			        				if Disruptor.SleepReady(orderDelay5, lastMoveOrder5) then
			        					for i = 1, 39 do
			        						elements3[i] = elements3[i+1]
			        					end	
			        					elements3[40] = Entity.GetOrigin(enemyAll)
			        					lastMoveOrder5 = os.clock()
			        				end
			        			end
			        		end

							local glimpseOrigin3 = elements3[1]
							local tempColor = Menu.GetValue(color)
							local tempColor2 = Menu.GetValue(color2)
							if (tempColor.r < 50) and (tempColor.g < 50) and (tempColor.b < 50) then
							    tempColor.r = 50
							...
							if not glimpseOrigin3 or elements3[1] ~= glimpseOrigin3 or not enemyAll or not Ability.IsReady(glimpse) then
								Particle.Destroy(glimpseParticle3)			
								glimpseParticle3 = 0
								glimpseOrigin3 = elements3[1]
								image3 = nil
							else
								if glimpseParticle3 == 0 and enemyAll then
									glimpseParticle3 = Particle.Create("particles\\ui_mouseactions\\drag_selected_ring.vpcf")				
								end
								if glimpseParticle3 ~= 0 then
									if Menu.IsEnabled(optionCircle) then
										Particle.SetControlPoint(glimpseParticle3, 0, glimpseOrigin3)
			    			        	Particle.SetControlPoint(glimpseParticle3, 1, Vector(tempColor.r, tempColor.g, tempColor.b))
			    			        	Particle.SetControlPoint(glimpseParticle3, 2, Vector(Menu.GetValue(slider)*1.0923, 255, 0))
			    			        	Particle.SetControlPoint(glimpseParticle3, 3, Vector(255 - tempColor.a, 0, 0))
			    			        end
			    			        if not Ability.IsReady(glimpse) or not Menu.IsEnabled(optionCircle) then
			    			        	Particle.SetControlPoint(glimpseParticle3, 0, nil)	
			    			        end	
			    			        if Menu.IsEnabled(optionLine) then
			    			        	x13, y13 = Renderer.WorldToScreen(glimpseOrigin3)
			    			        	x23, y23 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))
			    			        	Renderer.SetDrawColor(tempColor2.r, tempColor2.g, tempColor2.b)
			    			        	Renderer.DrawLine(x13, y13, x23, y23)
			    			        end
			    			        if Menu.IsEnabled(optionIcon) then
			    			        	x13, y13 = Renderer.WorldToScreen(glimpseOrigin3)
			    			        	x23, y23 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))	
			    			        	local image3 = Renderer.LoadImage("panorama/images/heroes/icons/"..NPC.GetUnitName(enemyAll).."_png.vtex_c")
			    			        	Renderer.SetDrawColor(255, 255, 255)
			    			        	Renderer.DrawImage(image3, x13, y13)
			    			        end	
								end
							end
						elseif i == 4 or i == 9 then
							if Menu.IsEnabled(optionGlimpseEnable) and glimpse and Ability.GetLevel(glimpse) >= 1 then
			        			if enemyAll then
			        				if Disruptor.SleepReady(orderDelay6, lastMoveOrder6) then
			        					for i = 1, 39 do
			        						elements4[i] = elements4[i+1]
			        					end	
			        					elements4[40] = Entity.GetOrigin(enemyAll)
			        					lastMoveOrder6 = os.clock()
			        				end
			        			end
			        		end

							local glimpseOrigin4 = elements4[1]
							local tempColor = Menu.GetValue(color)
							local tempColor2 = Menu.GetValue(color2)
							if (tempColor.r < 50) and (tempColor.g < 50) and (tempColor.b < 50) then
							    tempColor.r = 50
							...
							if not glimpseOrigin4 or elements4[1] ~= glimpseOrigin4 or not enemyAll or not Ability.IsReady(glimpse) then
								Particle.Destroy(glimpseParticle4)			
								glimpseParticle4 = 0
								glimpseOrigin4 = elements4[1]
								image4 = nil
							else
								if glimpseParticle4 == 0 and enemyAll then
									glimpseParticle4 = Particle.Create("particles\\ui_mouseactions\\drag_selected_ring.vpcf")				
								end
								if glimpseParticle4 ~= 0 then
									if Menu.IsEnabled(optionCircle) then
										Particle.SetControlPoint(glimpseParticle4, 0, glimpseOrigin4)
			    			        	Particle.SetControlPoint(glimpseParticle4, 1, Vector(tempColor.r, tempColor.g, tempColor.b))
			    			        	Particle.SetControlPoint(glimpseParticle4, 2, Vector(Menu.GetValue(slider)*1.0923, 255, 0))
			    			        	Particle.SetControlPoint(glimpseParticle4, 3, Vector(255 - tempColor.a, 0, 0))
			    			        end
			    			        if not Ability.IsReady(glimpse) or not Menu.IsEnabled(optionCircle) then
			    			        	Particle.SetControlPoint(glimpseParticle4, 0, nil)	
			    			        end	
			    			        if Menu.IsEnabled(optionLine) then
			    			        	x14, y14 = Renderer.WorldToScreen(glimpseOrigin4)
			    			        	x24, y24 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))
			    			        	Renderer.SetDrawColor(tempColor2.r, tempColor2.g, tempColor2.b)
			    			        	Renderer.DrawLine(x14, y14, x24, y24)
			    			        end
			    			        if Menu.IsEnabled(optionIcon) then
			    			        	x14, y14 = Renderer.WorldToScreen(glimpseOrigin4)
			    			        	x24, y24 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))	
			    			        	local image4 = Renderer.LoadImage("panorama/images/heroes/icons/"..NPC.GetUnitName(enemyAll).."_png.vtex_c")
			    			        	Renderer.SetDrawColor(255, 255, 255)
			    			        	Renderer.DrawImage(image4, x14, y14)
			    			        end	
								end
							end
						elseif i == 5 or i == 10 then
							if Menu.IsEnabled(optionGlimpseEnable) and glimpse and Ability.GetLevel(glimpse) >= 1 then
			        			if enemyAll then
			        				if Disruptor.SleepReady(orderDelay7, lastMoveOrder7) then
			        					for i = 1, 39 do
			        						elements5[i] = elements5[i+1]
			        					end	
			        					elements5[40] = Entity.GetOrigin(enemyAll)
			        					lastMoveOrder7 = os.clock()
			        				end
			        			end
			        		end

							local glimpseOrigin5 = elements5[1]
							local tempColor = Menu.GetValue(color)
							local tempColor2 = Menu.GetValue(color2)
							if (tempColor.r < 50) and (tempColor.g < 50) and (tempColor.b < 50) then
							    tempColor.r = 50
							...
							if not glimpseOrigin5 or elements5[1] ~= glimpseOrigin5 or not enemyAll or not Ability.IsReady(glimpse) then
								Particle.Destroy(glimpseParticle5)			
								glimpseParticle5 = 0
								glimpseOrigin5 = elements5[1]
								image5 = nil
							else
								if glimpseParticle5 == 0 and enemyAll then
									glimpseParticle5 = Particle.Create("particles\\ui_mouseactions\\drag_selected_ring.vpcf")				
								end
								if glimpseParticle5 ~= 0 then
									if Menu.IsEnabled(optionCircle) then
										Particle.SetControlPoint(glimpseParticle5, 0, glimpseOrigin5)
			    			        	Particle.SetControlPoint(glimpseParticle5, 1, Vector(tempColor.r, tempColor.g, tempColor.b))
			    			        	Particle.SetControlPoint(glimpseParticle5, 2, Vector(Menu.GetValue(slider)*1.0923, 255, 0))
			    			        	Particle.SetControlPoint(glimpseParticle5, 3, Vector(255 - tempColor.a, 0, 0))
			    			        end
			    			        if not Ability.IsReady(glimpse) or not Menu.IsEnabled(optionCircle) then
			    			        	Particle.SetControlPoint(glimpseParticle5, 0, nil)	
			    			        end	
			    			        if Menu.IsEnabled(optionLine) then
			    			        	x15, y15 = Renderer.WorldToScreen(glimpseOrigin5)
			    			        	x25, y25 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))
			    			        	Renderer.SetDrawColor(tempColor2.r, tempColor2.g, tempColor2.b)
			    			        	Renderer.DrawLine(x15, y15, x25, y25)
			    			        end
			    			        if Menu.IsEnabled(optionIcon) then
			    			        	x15, y15 = Renderer.WorldToScreen(glimpseOrigin5)
			    			        	x25, y25 = Renderer.WorldToScreen(Entity.GetOrigin(enemyAll))	
			    			        	local image5 = Renderer.LoadImage("panorama/images/heroes/icons/"..NPC.GetUnitName(enemyAll).."_png.vtex_c")
			    			        	Renderer.SetDrawColor(255, 255, 255)
			    			        	Renderer.DrawImage(image5, x15, y15)
			    			        end	
								end
							end										
						end	        	
