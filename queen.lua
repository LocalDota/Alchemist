...
--Предметы
	...
	local dagon2 = NPC.GetItem(myHero, "item_dagon_2")

	local dagon3 = NPC.GetItem(myHero, "item_dagon_3")

	local dagon4 = NPC.GetItem(myHero, "item_dagon_4")

	local dagon5 = NPC.GetItem(myHero, "item_dagon_5")
--------------------------------------------------------------------------------
	...
	if Menu.IsKeyDown(Queen.optionFullCombo) and enemy then
		...
		if linken and Ability.IsReady(linken) then
			...
			if Menu.IsEnabled(Queen.optionDagonL) then
				if Queen.Dagon(dagon, enemy, mana) == true then
					return
				end

				if Queen.Dagon2(dagon2, enemy, mana) == true then
					return
				end

				if Queen.Dagon3(dagon3, enemy, mana) == true then
					return
				end

				if Queen.Dagon4(dagon4, enemy, mana) == true then
					return
				end

				if Queen.Dagon5(dagon5, enemy, mana) == true then
					return
				end
			end	
		end					

		if Queen.SheepStick(sheepstick, enemy, mana) == true then
			return
		end

		if Queen.Bloodthorn(bloodthorn, enemy, mana) == true then
			return
		end

		if Queen.Orchid(orchid, enemy, mana) == true then
			return
		end

		if Queen.Veil(veilofdiscord, Entity.GetAbsOrigin(enemy), mana) == true then
			return
		end

		if Queen.Ethereal(etherealblade, enemy, mana) == true then
			return
		end

		if Queen.ShivasGuard(shivasguard, mana) == true then
			return
		end

		if Queen.Scream(scream, mana) == true then
			return
		end
		...
		if Queen.LotusOrb(lotusorb, myHero, mana) == true then
			return
		end
	

		if Queen.BlackKingBar(blackkingbar, mana) == true then
			return
		end

		if Queen.BladeMail(blademail, mana) == true then
			return
		end
...
function Queen.Scream(ability, mana)
	if Menu.IsEnabled(Queen.optionScream) and Ability.IsReady(ability) and Ability.IsCastable(ability, mana) then
...
function Queen.SheepStick(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionSheepStick) and item  and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
...
function Queen.Orchid(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionOrchidMalevolence) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, enemy)	 
    	return true
    end	  
end

function Queen.Mjollnir(item, myHero, mana)
	if Menu.IsEnabled(Queen.optionMjollnir) and item  and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, myHero)	 
    	return true
    end	  
end

function Queen.Veil(item, origin, mana)
	if Menu.IsEnabled(Queen.optionVeilOfDiscord) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastPosition(item, origin) 
    	return true
    end	  
end

function Queen.Bloodthorn(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionBloodthorn) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, enemy)	 
    	return true
    end	  
end

function Queen.Ethereal(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionEthereal) and item  and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, enemy)
    	return true	 
    end	  
end

function Queen.ShivasGuard(item, mana)
	if Menu.IsEnabled(Queen.optionShivasGuard) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastNoTarget(item)
    	return true	 
    end	  
end

function Queen.BlackKingBar(item, mana)
	if Menu.IsEnabled(Queen.optionBlackKingBar) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastNoTarget(item)	
    	return true 
    end	  
end

function Queen.BladeMail(item, mana)
	if Menu.IsEnabled(Queen.optionBladeMail) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastNoTarget(item)	 
    	return true
    end	  
end

function Queen.Satanic(item, mana)
	if Menu.IsEnabled(Queen.optionSatanic) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastNoTarget(item)	 
    	return true
    end	  
end

function Queen.LotusOrb(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionLotusOrb) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, enemy)	
    	return true 
    end	  
end
...
function Queen.Dagon(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionDagon) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, enemy)	
    	return true 
    end	  
end

function Queen.Dagon2(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionDagon) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, enemy)	
    	return true 
    end	  
...
function Queen.Dagon3(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionDagon) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, enemy)	
    	return true 
    end	  
...
function Queen.Dagon4(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionDagon) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, enemy)	
    	return true 
    end	  
...
function Queen.Dagon5(item, enemy, mana)
	if Menu.IsEnabled(Queen.optionDagon) and item and Ability.IsReady(item) and Ability.IsCastable(item, mana) then
    	Ability.CastTarget(item, enemy)	
    	return true 
    end	  
